...
async function validarFormulario() {
  ...
  const codigoAsistente = userData?.codigo_persona;

  if (!fecha || !hora || !codigoMedico || !codigoAsistente) {
  ...
  try {
    const res = await fetch('http://localhost:3000/api/citas/nueva/asistente', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ codigoAsistente, codigoMedico, fecha, hora, monto, anticipo })
    });
